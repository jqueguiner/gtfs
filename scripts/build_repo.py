...
def main():
    ...
    for f in feeds:
        url = f.get("producer_url") or f.get("hosted_url")
        if not url:
            continue
        fid = f.get("id") or url
        cc = (f.get("cc") or "XX").upper()
        if not re.fullmatch(r"[A-Z]{2}", cc):
            cc = "XX"
        city = f.get("city") or f.get("subdiv") or "national"
        agency = f.get("provider") or f.get("name") or fid
        cslug = (slug(city, "")[:40] or "unknown")
        # ascii base + short hash of the source id => always ascii, bounded, unique
        uid = hashlib.md5(str(fid).encode("utf-8")).hexdigest()[:8]
        aslug = (slug(agency, "")[:48] or "op") + "-" + uid
        d = os.path.join(ROOT, cc, cslug, aslug)
        os.makedirs(d, exist_ok=True)
        manifest = {
            "agency": {"name": agency, "country": cc, "subdivision": f.get("subdiv"), "city": f.get("city") or None},
            "feed": {"format": "gtfs", "url": url,
                     "mdb_id": fid, "license": f.get("license") or None},
            "bbox": f.get("bbox") or None,
            "source": f.get("source") or "mobility-database",
            "status": "active",
        }
        ...
        os.chmod(fp, 0o755)
        rows.append([cc, city, agency, url, fid, f.get("license"), "active"])
        pc = per_country[cc]
